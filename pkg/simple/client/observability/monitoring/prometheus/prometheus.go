...
func NewPrometheus(options *observability.MonitoringOptions) (monitoring.Interface, error) {
	cfg := api.Config{
...
func genMetricFilter(o monitoring.QueryOption) func(metric model.Metric) bool {
	if o != nil {
		if so, ok := o.(monitoring.SampleOption); ok {
			if so.NamespacedNameResourcesFilter != "" {
				namespacedPodsMap := make(map[string]struct{})
				for _, s := range strings.Split(so.NamespacedNameResourcesFilter, "|") {
