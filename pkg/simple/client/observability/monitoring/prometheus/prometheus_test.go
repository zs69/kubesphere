...
func TestGetNamedMetrics(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			client, _ := NewPrometheus(&observability.MonitoringOptions{Endpoint: srv.URL})
			result := client.GetNamedMetrics([]string{"cluster_cpu_utilisation"}, time.Now(), monitoring.SampleOption{Level: monitoring.LevelCluster})
...
func TestGetNamedMetricsOverTime(t *testing.T) {
	...
	for i, tt := range tests {
		t.Run(fmt.Sprintf("%d", i), func(t *testing.T) {
			...
			client, _ := NewPrometheus(&observability.MonitoringOptions{Endpoint: srv.URL})
			result := client.GetNamedMetricsOverTime([]string{"cluster_cpu_utilisation"}, time.Now().Add(-time.Minute*3), time.Now(), time.Minute, monitoring.SampleOption{Level: monitoring.LevelCluster})
			if diff := cmp.Diff(result, expected); diff != "" {
				t.Fatalf("%T differ (-got, +want): %s", expected, diff)
			}
		})
