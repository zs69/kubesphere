...
var PromQLs = map[string]string{
	...
	"pod_cpu_usage":                         `round(sum by (cluster,namespace, pod, qos) (irate(container_cpu_usage_seconds_total{job="kubelet", pod!="", image!=""}[5m])) * on (cluster, namespace, pod) group_left (qos) qos_owner_node:kube_pod_info:{namespace="default",owner_kind="ReplicaSet", owner_name=~"^elasticsearch-[^-]{1,10}$",pod=~"elasticsearch-0"},0.001)`,
	"pod_memory_usage":                      `sum by (cluster, namespace, pod, qos) (container_memory_usage_bytes{job="kubelet", pod!="", image!=""}) * on (cluster,namespace, pod) group_left (qos) qos_owner_node:kube_pod_info:{namespace="default",pod="elasticsearch-12345"}`,
	"pod_memory_usage_wo_cache":             `sum by (cluster, namespace, pod, qos) (container_memory_working_set_bytes{job="kubelet", pod!="", image!=""})  * on (cluster, namespace, pod) group_left (qos) qos_owner_node:kube_pod_info:{node="i-2dazc1d6",pod="elasticsearch-12345"}`,
	"pod_net_bytes_transmitted":             `round(sum by (cluster, namespace, pod, qos) (irate(container_network_transmit_bytes_total{pod!="", interface!~"^(cali.+|tunl.+|dummy.+|kube.+|flannel.+|cni.+|docker.+|veth.+|lo.*)", job="kubelet"}[5m])) * on (cluster, namespace, pod) group_left (qos) qos_owner_node:kube_pod_info:{namespace=~"logging|ks|",pod=~"elasticsearch-0|redis|"}, 0.001)`,
