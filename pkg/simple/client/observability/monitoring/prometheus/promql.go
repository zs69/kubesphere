...
var promQLTemplates = map[string]string{
	...
	"clusters_pod_qos_guaranteed_count":  `count(kube_pod_info{job="kube-state-metrics",$filter}and on(pod, namespace) (kube_pod_status_phase{job="kube-state-metrics",phase="Running",$filter} > 0) and on(pod, namespace) (container_memory_working_set_bytes{container="",pod!="",id !~ ".*(burstable|besteffort).*",$filter}))`,
	"clusters_pod_qos_burstable_count":   `count(kube_pod_info{job="kube-state-metrics",$filter}and on(pod, namespace) (kube_pod_status_phase{job="kube-state-metrics",phase="Running",$filter} > 0) and on(pod, namespace) (container_memory_working_set_bytes{container="",pod!="",id =~".*burstable.*",$filter}))`,
	"clusters_pod_qos_besteffort_count":  `count(kube_pod_info{job="kube-state-metrics",$filter}and on(pod, namespace) (kube_pod_status_phase{job="kube-state-metrics",phase="Running",$filter} > 0) and on(pod, namespace) (container_memory_working_set_bytes{container="",pod!="",id =~".*besteffort.*",$filter}))`,
	...
	"cluster_pod_qos_guaranteed_count":                   `count by(cluster)(kube_pod_info{job="kube-state-metrics",$filter}and on(pod, namespace) (kube_pod_status_phase{job="kube-state-metrics",phase="Running",$filter} > 0) and on(pod, namespace) (container_memory_working_set_bytes{container="",pod!="",id !~ ".*(burstable|besteffort).*",$filter}))`,
	"cluster_pod_qos_burstable_count":                    `count by(cluster)(kube_pod_info{job="kube-state-metrics",$filter}and on(pod, namespace) (kube_pod_status_phase{job="kube-state-metrics",phase="Running",$filter} > 0) and on(pod, namespace) (container_memory_working_set_bytes{container="",pod!="",id =~".*burstable.*",$filter}))`,
	"cluster_pod_qos_besteffort_count":                   `count by(cluster)(kube_pod_info{job="kube-state-metrics",$filter}and on(pod, namespace) (kube_pod_status_phase{job="kube-state-metrics",phase="Running",$filter} > 0) and on(pod, namespace) (container_memory_working_set_bytes{container="",pod!="",id =~".*besteffort.*",$filter}))`,
	...
	"cluster_gpu_memory_utilization": `sum by (cluster)(DCGM_FI_DEV_FB_USED{$filter}) / sum(DCGM_FI_DEV_FB_FREE{$filter} + DCGM_FI_DEV_FB_USED{$filter})`,
	...
	// unchecked gpu proml
	"node_gpu_utilization":        `round(avg(DCGM_FI_PROF_GR_ENGINE_ACTIVE * on (namespace, pod) group_left(node, host_ip, role) node_namespace_pod:kube_pod_info:{$filter}) by(node) / 100, 0.00001) or round(avg(DCGM_FI_DEV_GPU_UTIL* on (namespace, pod) group_left(node, host_ip, role) node_namespace_pod:kube_pod_info:{$filter}) by(node)/ 100, 0.00001)`,
	"node_gpu_usage":              `round(sum(DCGM_FI_PROF_GR_ENGINE_ACTIVE * on (namespace, pod) group_left(node, host_ip, role) node_namespace_pod:kube_pod_info:{$filter}) by(node) / 100, 0.00001) or round(sum(DCGM_FI_DEV_GPU_UTIL* on (namespace, pod) group_left(node, host_ip, role) node_namespace_pod:kube_pod_info:{$filter}) by(node)/ 100, 0.00001)`,
	"node_gpu_total":              `kube_node_status_capacity{resource="nvidia_com_gpu", $filter}`,
	"node_gpu_memory_utilization": `avg(DCGM_FI_DEV_FB_USED/(DCGM_FI_DEV_FB_FREE + DCGM_FI_DEV_FB_USED) * on(namespace , pod) group_left(node) (node_namespace_pod:kube_pod_info:{$filter})) by(node)`,
	"node_gpu_memory_usage":       `DCGM_FI_DEV_FB_USED * on(namespace , pod) group_left(node) (node_namespace_pod:kube_pod_info:{$filter}) * 1024 * 1024`,
	"node_gpu_memory_available":   `DCGM_FI_DEV_FB_FREE* on(namespace , pod) group_left(node) (node_namespace_pod:kube_pod_info:{$filter}) * 1024 * 1024`,
	"node_gpu_memory_total":       `sum((DCGM_FI_DEV_FB_FREE + DCGM_FI_DEV_FB_USED) * on(pod,namespace) group_left(node) node_namespace_pod:kube_pod_info:{$filter}) by(node) * 1024 * 1024`,
	"node_gpu_temp":               `round(DCGM_FI_DEV_GPU_TEMP* on(namespace , pod) group_left(node) (node_namespace_pod:kube_pod_info:{$filter}), 0.001)`,
	"node_gpu_power_usage":        `round(DCGM_FI_DEV_POWER_USAGE* on(namespace , pod) group_left(node) (node_namespace_pod:kube_pod_info:{$filter}), 0.001)`,
	...
	// unchecked gpu proml
	//	"workspace_gpu_usage":                  `round((sum by(cluster, workspace) (label_replace(DCGM_FI_PROF_GR_ENGINE_ACTIVE{exported_namespace!=""},"namespace","$1","exported_namespace","(.+)") * on(cluster, namespace) group_left(workspace) (kube_namespace_labels{$filter})) or  sum by(workspace) (label_replace(DCGM_FI_DEV_GPU_UTIL{exported_namespace!=""},"namespace","$1","exported_namespace","(.+)") * on(namespace) group_left(workspace) (kube_namespace_labels{$filter})) ) / 100,0.001)`,
	//	"workspace_gpu_memory_usage":           `(sum by (cluster, workspace) (label_replace(DCGM_FI_DEV_FB_USED{exported_namespace!=""},"namespace","$1","exported_namespace","(.+)") * on(cluster, namespace) group_left(workspace) (kube_namespace_labels{$filter}))) * 1024 * 1024`,
	...
	// unchecked gpu proml
	//"namespace_gpu_limit_hard":                   `sum by (cluster, namespace) (kube_resourcequota{resource="requests.nvidia.com/gpu", type="hard", namespace!=""} * on (namespace) group_left(workspace) kube_namespace_labels{$filter})`,
	//"namespace_gpu_usage":                        `round((sum by(namespace) (label_replace(DCGM_FI_PROF_GR_ENGINE_ACTIVE{exported_namespace!=""},"namespace","$1","exported_namespace","(.+)") * on(namespace) group_left(workspace) (kube_namespace_labels{$filter})) or  sum by(namespace) (label_replace(DCGM_FI_DEV_GPU_UTIL{exported_namespace!=""},"namespace","$filter","exported_namespace","(.+)") * on(namespace) group_left(workspace) (kube_namespace_labels{$filter})) ) / 100,0.001)`,
	//"namespace_gpu_memory_usage":                 `(sum by (cluster, namespace) (label_replace(DCGM_FI_DEV_FB_USED{exported_namespace!=""},"namespace","$1","exported_namespace","(.+)") * on(namespace) group_left(workspace) (kube_namespace_labels{$filter}))) * 1024 * 1024`,
	...
	// unchecked gpu proml
	//"workload_gpu_usage":                              `round((sum by(namespace, owner_kind, workload) ((label_replace(label_replace(DCGM_FI_PROF_GR_ENGINE_ACTIVE{exported_namespace!=""},"namespace","$filter","exported_namespace","(.+)"),"pod","$filter","exported_pod","(.+)")) * on(pod, namespace) group_left(workload, owner_kind) (label_join(label_replace(kube_pod_owner{owner_name="<none>",$2},"owner_name","$filter","pod","(.+)") or kube_pod_owner{owner_name!="<none>",$2}, "workload",":","owner_kind","owner_name")) )*  sum by(namespace, owner_kind, workload) (label_join(label_replace(kube_pod_owner{owner_name="<none>",$2},"owner_name","$filter","pod","(.+)") or kube_pod_owner{owner_name!="<none>",$2}, "workload",":","owner_kind","owner_name")))/100, 0.001) or round((sum by(namespace, owner_kind, workload) ((label_replace(label_replace(DCGM_FI_DEV_GPU_UTIL{exported_namespace!=""},"namespace","$filter","exported_namespace","(.+)"),"pod","$filter","exported_pod","(.+)")) * on(pod, namespace) group_left(workload, owner_kind) (label_join(label_replace(kube_pod_owner{owner_name="<none>",$2},"owner_name","$filter","pod","(.+)") or kube_pod_owner{owner_name!="<none>",$2}, "workload",":","owner_kind","owner_name")) )*  sum by(namespace, owner_kind, workload) (label_join(label_replace(kube_pod_owner{owner_name="<none>",$2},"owner_name","$filter","pod","(.+)") or kube_pod_owner{owner_name!="<none>",$2}, "workload",":","owner_kind","owner_name")))/100, 0.001)`,
	//"workload_gpu_memory_usage":                       `(sum by(namespace, owner_kind, workload) ((label_replace(label_replace(DCGM_FI_DEV_FB_USED{exported_namespace!=""},"namespace","$filter","exported_namespace","(.+)"),"pod","$filter","exported_pod","(.+)")) * on(pod, namespace) group_left(workload, owner_kind) (label_join(label_replace(kube_pod_owner{owner_name="<none>",$2},"owner_name","$filter","pod","(.+)") or kube_pod_owner{owner_name!="<none>",$2}, "workload",":","owner_kind","owner_name")))) * 1024 * 1024`,

	// pod
	"pod_cpu_usage":                        `round(sum by (cluster,namespace, pod, qos) (irate(container_cpu_usage_seconds_total{job="kubelet", pod!="", image!=""}[5m])) * on (cluster, namespace, pod) group_left (qos) qos_owner_node:kube_pod_info:{$filter},0.001)`,
	"pod_cpu_used_requests_utilisation":    `round(sum by (cluster,namespace, pod) (irate(container_cpu_usage_seconds_total{job="kubelet", pod!="", image!="",$filter}[5m])) / sum by (cluster,namespace,pod)(kube_pod_container_resource_requests{resource="cpu",$filter}),0.001)`,
	"pod_cpu_used_limits_utilisation":      `round(sum by (cluster,namespace, pod) (irate(container_cpu_usage_seconds_total{job="kubelet", pod!="", image!="",$filter}[5m])) / sum by (cluster,namespace,pod)(kube_pod_container_resource_limits{resource="cpu",$filter}),0.001)`,
	"pod_memory_usage":                     `sum by (cluster, namespace, pod, qos) (container_memory_usage_bytes{job="kubelet", pod!="", image!=""}) * on (cluster,namespace, pod) group_left (qos) qos_owner_node:kube_pod_info:{$filter}`,
	"pod_memory_usage_wo_cache":            `sum by (cluster, namespace, pod, qos) (container_memory_working_set_bytes{job="kubelet", pod!="", image!=""})  * on (cluster, namespace, pod) group_left (qos) qos_owner_node:kube_pod_info:{$filter}`,
	"pod_memory_used_requests_utilisation": `round(sum by (cluster, namespace, pod) (container_memory_working_set_bytes{job="kubelet", pod!="", image!="",$filter}) / sum by (cluster, namespace ,pod)(kube_pod_container_resource_requests{resource="memory",$filter}), 0.001)`,
	"pod_memory_used_limits_utilisation":   `round(sum by (cluster, namespace, pod) (container_memory_working_set_bytes{job="kubelet", pod!="", image!="",$filter}) / sum by (cluster, namespace, pod)(kube_pod_container_resource_limits{resource="memory",$filter}), 0.001)`,
	"pod_net_bytes_transmitted":            `round(sum by (cluster, namespace, pod, qos) (irate(container_network_transmit_bytes_total{pod!="", interface!~"^(cali.+|tunl.+|dummy.+|kube.+|flannel.+|cni.+|docker.+|veth.+|lo.*)", job="kubelet"}[5m])) * on (cluster, namespace, pod) group_left (qos) qos_owner_node:kube_pod_info:{$filter}, 0.001)`,
	"pod_net_bytes_received":               `round(sum by (cluster, namespace, pod, qos) (irate(container_network_receive_bytes_total{pod!="", interface!~"^(cali.+|tunl.+|dummy.+|kube.+|flannel.+|cni.+|docker.+|veth.+|lo.*)", job="kubelet"}[5m])) * on (cluster, namespace, pod) group_left (qos) qos_owner_node:kube_pod_info:{$filter}, 0.001)`,
	"pod_pvc_bytes_used":                   `max by (cluster, namespace, persistentvolumeclaim) (kubelet_volume_stats_used_bytes) * on (cluster, namespace, persistentvolumeclaim) group_left (storageclass) kube_persistentvolumeclaim_info{} * on (cluster, namespace, persistentvolumeclaim) group_left (pod) kube_pod_spec_volumes_persistentvolumeclaims_info{$filter}`,
	...
	// unchecked gpu proml
	// "pod_gpu_usage":        `round(((sum by(namespace, pod) (label_replace(label_replace(DCGM_FI_PROF_GR_ENGINE_ACTIVE{exported_namespace!=""},"namespace","$1","exported_namespace","(.+)"),"pod","$1","exported_pod","(.+)") *on (namespace, pod) group_left(owner_kind, owner_name) kube_pod_owner{$filter} * on (namespace, pod) group_left(node) kube_pod_info{$2})) * (sum by (namespace,pod) (kube_pod_info{}))) /100, 0.001) or round(((sum by(namespace, pod) (label_replace(label_replace(DCGM_FI_DEV_GPU_UTIL{exported_namespace!=""},"namespace","$1","exported_namespace","(.+)"),"pod","$1","exported_pod","(.+)") *on (namespace, pod) group_left(owner_kind, owner_name) kube_pod_owner{$filter} * on (namespace, pod) group_left(node) kube_pod_info{})) * (sum by (namespace,pod) (kube_pod_info{$2}))) /100, 0.001)`,
	// "pod_gpu_memory_usage": `(sum by(namespace, pod) (label_replace(label_replace(DCGM_FI_DEV_FB_USED{exported_namespace!=""},"namespace","$1","exported_namespace","(.+)"),"pod","$1","exported_pod","(.+)")) *on (namespace, pod) group_left(owner_kind, owner_name) kube_pod_owner{$filter} * on (namespace, pod) group_left(node) kube_pod_info{$2}) * 1024 * 1024 `,
	...
	// unchecked gpu proml
	// "container_gpu_usage":        `round((sum by(cluster, namespace, pod, container, device, gpu) (label_replace(label_replace(label_replace(DCGM_FI_PROF_GR_ENGINE_ACTIVE{$filter},"namespace","$filter","exported_namespace","(.+)"),"pod","$filter","exported_pod","(.+)"),"container","$filter","exported_container","(.+)")) or sum by(namespace, pod, container, device, gpu) (label_replace(label_replace(label_replace(DCGM_FI_DEV_GPU_UTIL{$filter},"namespace","$1","exported_namespace","(.+)"),"pod","$1","exported_pod","(.+)"),"container","$1","exported_container","(.+)")) )/100 , 0.001)`,
	// "container_gpu_memory_usage": `(sum by(cluster, namespace, pod, container) (label_replace(label_replace(label_replace(DCGM_FI_DEV_FB_USED{$filter},"namespace","$1","exported_namespace","(.+)"),"pod","$filter","exported_pod","(.+)"),"container","$filter","exported_container","(.+)") )) * 1024 * 1024`,
