...
var PromQLs = map[string]string{
	...
	"pod_cpu_usage":                         `round(sum by (namespace, pod) (irate(container_cpu_usage_seconds_total{job="kubelet", pod!="", image!=""}[5m])), 0.001) *  on (namespace, pod) group_left (qos,owner_kind,owner_name,node) qos_owner_node:kube_pod_info:{owner_kind="ReplicaSet", owner_name=~"^elasticsearch-[^-]{1,10}$",pod=~"elasticsearch-0", namespace="default"}`,
	"pod_memory_usage":                      `sum by (namespace, pod) (container_memory_usage_bytes{job="kubelet", pod!="", image!=""}) * on (namespace, pod) group_left (qos,owner_kind,owner_name,node) qos_owner_node:kube_pod_info:{pod="elasticsearch-12345", namespace="default"}`,
	"pod_memory_usage_wo_cache":             `sum by (namespace, pod) (container_memory_working_set_bytes{job="kubelet", pod!="", image!=""}) * on (namespace, pod) group_left (qos,owner_kind,owner_name,node) qos_owner_node:kube_pod_info:{pod="elasticsearch-12345", node="i-2dazc1d6"}`,
	"pod_net_bytes_transmitted":             `sum by (namespace, pod) (irate(container_network_transmit_bytes_total{pod!="", interface!~"^(cali.+|tunl.+|dummy.+|kube.+|flannel.+|cni.+|docker.+|veth.+|lo.*)", job="kubelet"}[5m])) * on (namespace, pod) group_left (qos,owner_kind,owner_name,node) qos_owner_node:kube_pod_info:{namespace=~"logging|ks",pod=~"elasticsearch-0|redis"}`,
