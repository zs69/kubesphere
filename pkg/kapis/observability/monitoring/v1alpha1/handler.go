...
func NewHandler(k kubernetes.Interface, monitoringClient monitoring.Interface, f informers.InformerFactory) *handler {

	return &handler{
		k:  k,
		mo: model.NewMonitoringOperator(monitoringClient, k, f),
	}
}
...
func (h handler) handleLabelValuesQuery(req *restful.Request, resp *restful.Response) {
	var res model.LabelValues
	params := parseRequestParams(req)
	if params.label == "" || params.start == "" || params.end == "" {
		api.HandleBadRequest(resp, nil, errors.New("required fields are missing: [label, start, end]"))
		return
	}
	...
	if params.clusterName != "" {
		matches = append(matches, fmt.Sprintf(`{cluster="%s"}`, params.clusterName))
	} else if params.clusterResourcesFilter != "" {
		matches = append(matches, fmt.Sprintf(`{cluster=~"%s"}`, params.clusterResourcesFilter))
	}

	if params.namespaceName != "" {
		matches = append(matches, fmt.Sprintf(`{namespace="%s"}`, params.namespaceName))
	} else if params.namespaceResourcesFilter != "" {
		matches = append(matches, fmt.Sprintf(`{namespace=~"%s"}`, params.namespaceResourcesFilter))
	}

	res = h.mo.GetLabelValues(params.label, matches, opt.start, opt.end)
	resp.WriteAsJson(res)
}
