...
func addAllControllers(mgr manager.Manager, client k8s.Client, informerFactory informers.InformerFactory,
	...
	if cmOptions.IsControllerEnabled("license") {
		lc := license.NewLicenseController(client.Config(), informerFactory.KubernetesSharedInformerFactory(),
			informerFactory.KubeSphereSharedInformerFactory(), cmOptions.MultiClusterOptions.Enable, stopCh)
		addControllerWithSetup(mgr, "license", lc)
	}
	...
	if cmOptions.IsControllerEnabled("manifest") {
		manifestReconciler := &manifest.ManifestReconciler{
			KsFactory:          informerFactory.KubeSphereSharedInformerFactory(),
			MultiClusterEnable: cmOptions.MultiClusterOptions.Enable,
		}
		addControllerWithSetup(mgr, "manifest", manifestReconciler)
	}
